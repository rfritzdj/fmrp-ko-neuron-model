...
# =============================================================================
# *** Default fixed synaptic strengths used across all cases unless swept ***
# g_AMPA default = 10.0, g_NMDA default = 5.0 (per user spec)
# Pf_AMPA / Pf_NMDA are the default Ca-influx scaling fractions for the
# synaptic currents (used by Cases 1-4, and as the "off" sweep partner in
# Cases 5/6 below). They are now passed explicitly into the deriv/solver
# functions (no longer baked-in globals) so Cases 5/6 can sweep them.
# =============================================================================
G_AMPA_DEFAULT = 10.0
G_NMDA_DEFAULT = 5.0
...
# --- Sanity-check plotting flag ---
# *** NEW: optional simulation+spike-detection sanity check plot ***
# Default True per request. WARNING: with PLOT_EVERY_CELL=True this will
# generate one figure per (r,c) grid cell PER CASE. With grid_res=10 and
# 6 cases that is up to 600 figures - this will be slow and memory heavy.
# Set to False, or subsample (see SANITY_PLOT_STRIDE below) if this is
# too slow in practice.
PLOT_SANITY_CHECK = False
SANITY_PLOT_STRIDE = 1   # set >1 to only plot every Nth (r,c) cell, e.g. 3 -> ~1/9 of cells

# =============================================================================
# *** NEW: Frequency decay rate fitting method switch ***
# Method 1 (default): your original fit, f(t) = f_ss + (f_0-f_ss)*exp(-t/tau_f),
#   with f_0/f_ss FIXED to the measured first/last instantaneous frequency
#   (tau_f is the only free parameter). Fit via scipy.optimize.curve_fit
#   (nonlinear least squares), with rail-rejection if tau_f lands within 5%
#   of tau_min/tau_max.
# Method 2 (eFEL-style): pure single exponential decay TO ZERO,
#   f(t) = a*exp(-t/tau_f), no steady-state offset. Linearized as
#   ln(f) = ln(a) - t/tau_f and fit via scipy.stats.linregress (ordinary
#   least squares in log-space) - faster, can't fail to converge, can't get
#   stuck at a bound. BUT this model is only appropriate for cells that
#   genuinely adapt toward ~0 Hz; for tonic-firing cells whose steady-state
#   frequency is far from zero, forcing a decay-to-zero fit will bias tau_f
#   (typically too LARGE) even when the fit's R^2 looks acceptable. The
#   second return value for method 2 is R^2 (goodness of fit), NOT f_ss -
#   there is no f_ss in this model. Check R^2 before trusting a method-2 fit.
# Switch with FREQ_DECAY_METHOD below; the one flag controls every call site
# in this script (extract_metrics passes it straight through).
# =============================================================================
FREQ_DECAY_METHOD = 2   # 1 = original f_ss-anchored nonlinear fit, 2 = eFEL-style log-linregress decay-to-zero

# =============================================================================
# *** Synaptic onset delay relative to step current onset ***
# Set to 0.0 to reproduce the original (simultaneous-onset) behavior exactly.
# Set to e.g. 50.0 or 100.0 (ms) to test whether AMPA's effect survives once
# its fast conductance pulse no longer collides with spike-train initiation.
# =============================================================================
SYN_ONSET_DELAY_MS = 0   # <-- change this to sweep delay values
...
# --- Numba Compiled Derivatives ---
# *** CHANGED: Pf_AMPA, Pf_NMDA are now explicit arguments (previously
# fixed globals) so Cases 5/6 can sweep them through the solver. ***
@njit
...
# *** CHANGED: default threshold set to -10.0 (was -5.0) to match the
# value extract_metrics has always actually called this with. The two
# were previously inconsistent (this default vs. the override below);
# now they agree everywhere. ***
#
# *** CHANGED: now supports two fitting methods via the `method` argument
# (defaults to the module-level FREQ_DECAY_METHOD flag above, so existing
# call sites that don't pass `method` explicitly automatically follow
# whatever the global switch is set to).
#
# method=1 (your original): f(t) = f_ss + (f_0-f_ss)*exp(-t/tau_f), with
#   f_0/f_ss fixed to the measured first/last instantaneous frequency.
#   Nonlinear fit via curve_fit, rail-rejection preserved exactly as before.
#   Second return value = f_ss_fixed (the measured steady-state frequency).
#
# method=2 (eFEL-style): f(t) = a*exp(-t/tau_f), decaying to ZERO (no f_ss
#   term at all). Linearized as ln(f) = ln(a) - t/tau_f and fit by ordinary
#   least squares (scipy.stats.linregress) in log-frequency space. Faster
#   and can't fail to converge, but only a good model for cells that
#   genuinely adapt toward ~0 Hz - for tonic cells with f_ss far from zero,
#   forcing this model biases tau_f (usually too large) even when R^2 looks
#   fine. Second return value = R^2 of the log-linear fit (NOT f_ss - this
#   model has no steady-state parameter). Inspect R^2 before trusting it.
#
# Returns (unchanged 4-tuple shape regardless of method, so extract_metrics
# and every downstream array assignment keep working without modification):
#   tau_f, second_value, n_spikes_detected, spike_idx
#   where second_value is f_ss for method 1, R^2 for method 2.
...
# =============================================================================
# --- NEW: Sanity-check plots - simulated trace + detected spikes overlay ---
# Default ON (PLOT_SANITY_CHECK = True). WARNING: this produces one figure
# per (r,c) grid cell PER CASE (up to grid_res^2 * 6 figures). Use
# SANITY_PLOT_STRIDE > 1, or set PLOT_SANITY_CHECK = False, to cut this down.
# =============================================================================
if PLOT_SANITY_CHECK:
    total_plots = sum(
        len(range(0, grid_res, SANITY_PLOT_STRIDE)) * len(range(0, grid_res, SANITY_PLOT_STRIDE))
        for _ in range(1, 7)
    )
    print(f"PLOT_SANITY_CHECK is True: generating up to {total_plots} figures "
          f"(grid_res={grid_res}, stride={SANITY_PLOT_STRIDE}, 6 cases). "
          f"This may be slow - set PLOT_SANITY_CHECK=False or increase "
          f"SANITY_PLOT_STRIDE if this is impractical.")

    for case_idx in range(1, 7):
        meta = case_meta[case_idx]
        for r in range(0, grid_res, SANITY_PLOT_STRIDE):
            for c in range(0, grid_res, SANITY_PLOT_STRIDE):
                V_trace = V_traces[case_idx][r, c, :]
                peaks = spike_indices[case_idx][r][c]

                fig_s, ax_s = plt.subplots(figsize=(8, 3))
                ax_s.plot(tspan, V_trace, color='k', linewidth=0.7, label='V (mV)')
                if peaks is not None and len(peaks) > 0:
                    ax_s.plot(tspan[peaks], V_trace[peaks], 'r^', markersize=5, label='Detected spikes')
                ax_s.axhline(-10.0, color='gray', linestyle='--', linewidth=0.6, label='Threshold (-10 mV)')
                ax_s.set_xlabel('Time (ms)')
                ax_s.set_ylabel('V (mV)')
                ax_s.set_title(f"Case {case_idx} | {meta['ylabel']}={meta['y_vec'][r]:.2f}, "
                               f"{meta['xlabel']}={meta['x_vec'][c]:.2f} | n_spikes={len(peaks) if peaks is not None else 0}",
                               fontsize=9)
...
# =============================================================================
# --- Diagnostic Plot: tau_f vs spike count, AMPA vs NMDA ---
# =============================================================================
fig2, axes2 = plt.subplots(1, 2, figsize=(14, 6))

# Case 2 = AMPA conductance sweep (tau_AMPA x g_AMPA), constant tau_NMDA
tau_f_ampa = results[2][3, :, :].flatten()
n_spk_ampa = results[2][4, :, :].flatten()
g_ampa_grid = np.tile(g_AMPA_vals, (grid_res, 1)).flatten()

valid_ampa = ~np.isnan(tau_f_ampa)
sc1 = axes2[0].scatter(n_spk_ampa[valid_ampa], tau_f_ampa[valid_ampa],
                        c=g_ampa_grid[valid_ampa], cmap='viridis', s=25, alpha=0.7)
axes2[0].set_xlabel('N spikes used in tau fit', fontsize=11)
axes2[0].set_ylabel('Fitted $\\tau_f$ (s)', fontsize=11)
axes2[0].set_title('AMPA sweep (Case 2)\ncolor = $g_{AMPA}$ (nS)', fontsize=12, fontweight='bold')
cbar1 = fig2.colorbar(sc1, ax=axes2[0])
cbar1.set_label('$g_{AMPA}$ (nS)', fontsize=10)
axes2[0].grid(alpha=0.3)

# Case 4 = AMPA conductance x NMDA tau sweep, constant tau_AMPA
tau_f_nmda = results[4][3, :, :].flatten()
n_spk_nmda = results[4][4, :, :].flatten()
g_ampa_grid_4 = np.tile(g_AMPA_vals, (grid_res, 1)).flatten()

valid_nmda = ~np.isnan(tau_f_nmda)
sc2 = axes2[1].scatter(n_spk_nmda[valid_nmda], tau_f_nmda[valid_nmda],
                        c=g_ampa_grid_4[valid_nmda], cmap='viridis', s=25, alpha=0.7)
axes2[1].set_xlabel('N spikes used in tau fit', fontsize=11)
axes2[1].set_ylabel('Fitted $\\tau_f$ (s)', fontsize=11)
axes2[1].set_title('AMPA conductance x NMDA tau sweep (Case 4)\ncolor = $g_{AMPA}$ (nS)', fontsize=12, fontweight='bold')
cbar2 = fig2.colorbar(sc2, ax=axes2[1])
cbar2.set_label('$g_{AMPA}$ (nS)', fontsize=10)
axes2[1].grid(alpha=0.3)

fig2.suptitle('Diagnostic: is $\\tau_f$ driven by spike count or by synaptic strength?',
              fontsize=13, fontweight='bold')
plt.tight_layout()
plt.show()

# Quick numeric check: correlation between tau_f and n_spikes within the valid region.
if valid_ampa.sum() > 2:
    corr_ampa = np.corrcoef(n_spk_ampa[valid_ampa], tau_f_ampa[valid_ampa])[0, 1]
    print(f"AMPA sweep: corr(tau_f, n_spikes) = {corr_ampa:.3f}  (n={valid_ampa.sum()} valid cells)")
if valid_nmda.sum() > 2:
    corr_nmda = np.corrcoef(n_spk_nmda[valid_nmda], tau_f_nmda[valid_nmda])[0, 1]
    print(f"NMDA sweep: corr(tau_f, n_spikes) = {corr_nmda:.3f}  (n={valid_nmda.sum()} valid cells)")
